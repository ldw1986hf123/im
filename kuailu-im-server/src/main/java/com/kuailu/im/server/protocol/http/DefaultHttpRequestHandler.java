...
import com.kuailu.im.core.http.listener.IHttpServerListener;
import com.kuailu.im.core.http.session.HttpSession;
import com.kuailu.im.core.utils.FileUtils;
import com.kuailu.im.server.protocol.http.mvc.Routes;
import com.kuailu.im.server.util.ClassUtils;
import com.kuailu.im.server.util.HttpResps;
		...
		try {
			...
			if (method != null) {
				...
				if (fileCache != null) {
					...
					String root =FileUtils.getJarPath()+httpConfig.getPageRoot()  ;
					if(!FileUtil.exist(FileUtils.getJarPath()+httpConfig.getPageRoot())){
						root=FileUtil.getAbsolutePath( FileUtil.getAbsolutePath(httpConfig.getPageRoot()));
					}
