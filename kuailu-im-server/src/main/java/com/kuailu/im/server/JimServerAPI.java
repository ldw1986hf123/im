...
import com.kuailu.im.core.cluster.ImCluster;
...
public class JimServerAPI implements ImConst {
	...
	public static boolean send(ImChannelContext imChannelContext, ImPacket imPacket){
		...
		boolean sendResult=Tio.send(imChannelContext.getTioChannelContext(), convertPacket);
		return sendResult;
	}
	...
	public static void bindUserListToGroup(Collection<String> userList, String groupId) {
		List<ImChannelContext> channelContextList = JimServerAPI.getByGroup(groupId);
		userList.stream()
				.flatMap(u -> JimServerAPI.getByUserId(u).stream())
				.filter(
						imChannelContext1 -> {
							if (channelContextList.contains(imChannelContext1)) {
								log.debug(
										"这个用户已经的终端已经在群聊中了，过滤掉吧:groupId:{} userId:{},terminal node:{}",
										groupId,
										imChannelContext1.getUserId(),
										imChannelContext1.getClientNode().toString());
								return false;
							} else {
								return true;
							}
						})
				.forEach(imContext -> JimServerAPI.bindGroup(imContext, groupId));
	}
	/**
	 * 绑定用户(如果配置了回调函数执行回调) 已废弃
	 * @param imChannelContext IM通道上下文
	 * @param userId 用户ID
	 */
	/*@Deprecated
...
//			user = UserDto.newBuilder().userId(userId).status(UserStatusType.ONLINE.getStatus()).build();
		}
	 ...
	 * @param user 绑定用户信息
	 */
	public static boolean bindUser(ImChannelContext imChannelContext, UserDto user){
	 ...
	 */
	@Deprecated
	public static boolean unbindUser(UserDto user){
		if(Objects.isNull(user)|| StringUtils.isEmpty(user.getUserId())){
			log.error("user or userId is null");
			return false;
		}
		...
		try{
			...
			for (ChannelContext channelContext :  channelContexts){
				...
				UserDto existUser = imChannelContext.getSessionContext().getImClientNode().getUser();
	...
	public static boolean bindGroup(ImChannelContext imChannelContext, String groupId){
		...
		return bindGroup(imChannelContext,groupDto);
	}
	 ...
	 * @param group 绑定群组对象
	 */
	public static boolean bindGroup(ImChannelContext imChannelContext, GroupDto group){
		try {
